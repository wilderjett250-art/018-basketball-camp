<script setup lang="ts">
    ...
    const AddEdit=defineAsyncComponent(()=>import('@/views/xunlianyingxinxi/add-edit.vue'));
    ...
    const editRef=ref();
        ...
        function  yuyueguanliCrossAddOrUpdateHandler(row, type, statusColumnName, tips, statusColumnValue) {
          ...
          Session.set('tips', tips);
           yuyueguanliCrossaddOrUpdate.value.open('我要预约',type,row);
    ...
    <el-card shadow="never" class="index">
        ...
        <div class="layout-padding">
            <el-form inline :model="query" label-suffix=":">
                  ...
                  <el-button v-if="isAuth('xunlianyingxinxi','删除') && selectedPets.length > 0" icon="ele-Delete" @click="handleBatchDelete" type="danger">多选删除</el-button>
                    <el-button v-if="isAuth('xunlianyingxinxi','新增')" icon="ele-Plus" type="success" @click="add()">新增</el-button>
                                                                            </el-form-item>
            </el-form>
          ...
          <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">训练营图片</th>
                      ...
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">招收人数</th>
            
          <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">详情</th>
    
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">开放时间</th>
            
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">赞</th>
            
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">踩</th>
        ...
        <span class="truncate max-w-[120px] block">
           ...
           <div v-else>无图片</div>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.zhaoshourenshu }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.zhaoshourenshu }}
                              </div>
                            </td>
        ...
        <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 rich-text-content">
          <div v-html="pet.xiangqing"></div>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.kaifangshijian }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.kaifangshijian }}
                              </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.thumbsupnum }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.thumbsupnum }}
                              </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.crazilynum }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.crazilynum }}
                              </div>
                            </td>
                    ...
                    <td class="px-6 py-4">
                      <div class="flex space-x-3">
                        <button v-if="isAuth('xunlianyingxinxi','修改')" @click.stop="handleEdit(pet)" class="px-3 py-1 text-xs bg-green-500 text-white hover:bg-green-600 !rounded-button whitespace-nowrap">
                          <i class="fas fa-edit mr-1"></i>编辑
                        </button>
                        <button v-if="isAuth('xunlianyingxinxi','删除')" @click.stop="handleDelete(pet)" class="px-3 py-1 text-xs bg-red-500 text-white hover:bg-red-600 !rounded-button whitespace-nowrap">
                          <i class="fas fa-trash mr-1"></i>删除
                        </button>
                                                      <button  v-if="isAuth('xunlianyingxinxi','我要预约')"
                                                                     @click="yuyueguanliCrossAddOrUpdateHandler(row,'cross','','','')"
                                                                     class="px-3 py-1 text-xs bg-red-500 text-white hover:bg-red-600 !rounded-button whitespace-nowrap">
                              <i class="fas fa-edit mr-1"></i> 我要预约
                            </button>
