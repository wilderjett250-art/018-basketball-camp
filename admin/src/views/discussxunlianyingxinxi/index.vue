<script setup lang="ts">
    ...
    const AddEdit=defineAsyncComponent(()=>import('@/views/discussxunlianyingxinxi/add-edit.vue'));
    ...
    function getDateList(){
        const params={
            ...
            limit: state.page.size,
                                                                                                                                                            ...
                                                                                                                                                            request({
            url:'discussxunlianyingxinxi/page',
            method:'get',
            params
        }).then(({data})=>{
    ...
    function handleDelete(row:any){
        ...
        request({
            url:`discussxunlianyingxinxi/delete`,
            method:'post',
            data:[row.id]
        }).then(({data})=>{
    ...
    <el-card shadow="never" class="index">
        ...
        <div class="layout-padding">
            <el-form inline :model="query" label-suffix=":">
                  ...
                  <el-button v-if="isAuth('discussxunlianyingxinxi','删除') && selectedPets.length > 0" icon="ele-Delete" @click="handleBatchDelete" type="danger">多选删除</el-button>
                    <el-button v-if="isAuth('discussxunlianyingxinxi','新增')" icon="ele-Plus" type="success" @click="add()">新增</el-button>
                                                                            </el-form-item>
            </el-form>
                ...
                <table class="w-full h-400 border-collapse">
                  ...
                  <tr class="border-b border-gray-100">
                    <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors">
                      ...
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">关联表id</th>
            
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">用户id</th>
            
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">用户名</th>
            
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">评论内容</th>
            
                      <th class="px-6 py-4 text-left text-sm font-medium text-gray-500 transition-colors whitespace-nowrap">回复内容</th>
                  ...
                  <tr v-for="pet in tableList" :key="pet.id" class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 text-sm group relative">
                      ...
                      >
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.refid }}</span>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.nickname }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.nickname }}
                              </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.content }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.content }}
                              </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm group relative">
                              <span class="truncate max-w-[120px] block">{{ pet.reply }}</span>
                              <div class="absolute left-0 top-full z-50 bg-gray-900 text-white p-2 rounded whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                {{ pet.reply }}
                              </div>
                            </td>
                    ...
                    <td class="px-6 py-4">
                      <div class="flex space-x-3">
                        <button v-if="isAuth('discussxunlianyingxinxi','修改')" @click.stop="handleEdit(pet)" class="px-3 py-1 text-xs bg-green-500 text-white hover:bg-green-600 !rounded-button whitespace-nowrap">
                          <i class="fas fa-edit mr-1"></i>编辑
                        </button>
                        <button v-if="isAuth('discussxunlianyingxinxi','删除')" @click.stop="handleDelete(pet)" class="px-3 py-1 text-xs bg-red-500 text-white hover:bg-red-600 !rounded-button whitespace-nowrap">
