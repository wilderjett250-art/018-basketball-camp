        ...
        function getDataList(){
            ...
            const params={
                ...
                limit: state.page.size,
                                                                                                        ...
                                                                                                        if (state.searchForm.qicaimingcheng != '' && state.searchForm.qicaimingcheng != undefined) {
                        params['qicaimingcheng'] = '%' + state.searchForm.qicaimingcheng + '%'
                    }
                                                                                                                

            request({
                url:'qicaiguanli/list',
                method:'get',
                params
            }).then((data)=>{
...
<template ref="ragRefInstance">
  ...
  <div>
    ...
    <div class="py-20 bg-white">
      <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl font-bold text-center mb-4">器材管理</h2>
        <div class="max-w-4xl mx-auto mb-12">
          <div class="flex items-center gap-4">

                                                                                                                                                                                                                                                      <div class="relative flex-1">
              <input type="text" placeholder="器材名称" v-model="searchForm.qicaimingcheng" class="w-full px-4 py-2 border-2 border-gray-200 !rounded-button focus:outline-none focus:border-blue-500 pl-10 text-sm">
  ...
  <router-link :to="`/qicaiguanlidetail/`+item.id" class="shadow">
              <img :src="item.qicaitupian" class="w-full h-64 object-cover object-center transition-transform duration-300 group-hover:scale-110">
</router-link>
            </div>
                                                                                            <h3 class="mt-4 text-lg font-semibold">{{ item.qicaimingcheng }}</h3>
                                                                                                                </div>
        </div>
      </div>
