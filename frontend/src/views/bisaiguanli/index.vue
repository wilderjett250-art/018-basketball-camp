        ...
        function getDataList(){
            ...
            const params={
                ...
                limit: state.page.size,
                                                                                                        ...
                                                                                                        if (state.searchForm.changdimingcheng != '' && state.searchForm.changdimingcheng != undefined) {
                        params['changdimingcheng'] = '%' + state.searchForm.changdimingcheng + '%'
                    }
...
<template ref="ragRefInstance">
  ...
  <div>
    ...
    <div class="py-20 bg-white">
      <div class="max-w-7xl mx-auto px-4">
        <h2 class="text-3xl font-bold text-center mb-4">比赛管理</h2>
        <div class="max-w-4xl mx-auto mb-12">
          <div class="flex items-center gap-4">

                                                                                                                                                                                                                                                      <div class="relative flex-1">
              <input type="text" placeholder="场地名称" v-model="searchForm.changdimingcheng" class="w-full px-4 py-2 border-2 border-gray-200 !rounded-button focus:outline-none focus:border-blue-500 pl-10 text-sm">
  ...
  <router-link :to="`/bisaiguanlidetail/`+item.id" class="shadow">
              <img :src="item.changditupian" class="w-full h-64 object-cover object-center transition-transform duration-300 group-hover:scale-110">
</router-link>
            </div>
                                                                                            <h3 class="mt-4 text-lg font-semibold">{{ item.changdimingcheng }}</h3>
                                                                                                                                                                    </div>
        </div>
      </div>
