<script setup lang="ts">
        ...
        if (state.type == 'cross') {
            ...
            for (var o in obj) {
                            ...
                            continue;
                                                                                ...
                                                                                if (o == 'name') {
                            state.formData.name = obj[o];
                            state.roisfalg.name = true;
                            continue;
                        }
                                                                                if (o == 'sl') {
                            state.formData.sl = obj[o];
                            state.roisfalg.sl = true;
                            continue;
                        }
                                                                                if (o == 'crkzt') {
                            state.formData.crkzt = obj[o];
                            state.roisfalg.crkzt = true;
                            continue;
                        }
                                                }
    ...
    function submitForm(){
        formRef.value?.validate((valid:boolean)=>{
            ...
            state.loadding=true;
            request({
                url:`crk/${!state.formData.id ? "save" : "update"}`,
                method:'post',
                data:state.formData
            }).then(({data})=>{
